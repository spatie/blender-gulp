...
gulp.task('fix-cs-js', function() {
    shell.task(config.paths.cs.js.map(path => `jscs fix ${path}` ))
})

gulp.task('fix-cs-php', function() {
    shell.task(config.paths.cs.php.map(path => `php-cs-fixer fix ${path}` ))
